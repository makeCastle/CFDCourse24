...
public:
	...
	ATestTransport2Worker(size_t n_cells) : _grid(-1.0, 1.0, -1.0, 1.0, n_cells, n_cells), _hx(1.0 / n_cells), _hy(1.0 / n_cells), _u(_grid.n_points()) {
		for (size_t i = 0; i < _grid.n_points(); ++i) {
...
///////////////////////////////////////////////////////////////////////////////
// Explicit transport 2D solver
///////////////////////////////////////////////////////////////////////////////
class TestTransport2WorkerExplicit : public ATestTransport2Worker {
public:
	TestTransport2WorkerExplicit(size_t n_cells) : ATestTransport2Worker(n_cells) {}

private:
	void impl_step(double tau, double U, double V) override {
		std::vector<double> u_old(_u);
		_u[0] = exact_solution(_grid.point(0).x(), _grid.point(0).y());
		size_t upx = 0;
		size_t upy = 0;
		size_t pointsx = (_grid.point(_grid.n_points() - 1).x() - _grid.point(0).x()) / _hx;
		size_t pointsy = (_grid.point(_grid.n_points() - 1).y() - _grid.point(0).y()) / _hy;

		for (size_t i = 1; i < pointsx; ++i) {
			for (size_t j = 1; j < pointsy; ++i) {
