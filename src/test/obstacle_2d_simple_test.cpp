...
struct Obstacle2DSimpleWorker{
	Obstacle2DSimpleWorker(double Re, const RegularGrid2D& grid, double E);
...
Obstacle2DSimpleWorker::Obstacle2DSimpleWorker(double Re, const RegularGrid2D& grid, double E):
	_grid(grid),
...
std::vector<double> Obstacle2DSimpleWorker::compute_phi() const{
...
void Obstacle2DSimpleWorker::initialize(){
	...
	// build pressure
	std::vector<double> p(p_size(), 0);

	set_uvp(u, v, p);
};

void Obstacle2DSimpleWorker::initialize_saver(bool save_exact_fields, std::string stem){
...
double Obstacle2DSimpleWorker::set_uvp(const std::vector<double>& u, const std::vector<double>& v, const std::vector<double>& p){
	_u = u;
	_v = v;
	_p = p;
	assemble_u_slae();
...
double Obstacle2DSimpleWorker::step(){
...
void Obstacle2DSimpleWorker::save_current_fields(size_t iter){
...
void Obstacle2DSimpleWorker::assemble_p_stroke_solver(){
...
void Obstacle2DSimpleWorker::assemble_u_slae(){
	...
	for (size_t j=0; j< _grid.ny(); ++j){
		...
		// right boundary: u*du/dx = 0
		{
			...
			add_to_mat(index_right, {_grid.nx(), j}, 1.0 + coef);
			add_to_mat(index_right, {_grid.nx()-1, j}, -coef);
...
void Obstacle2DSimpleWorker::assemble_v_slae(){
...
std::vector<double> Obstacle2DSimpleWorker::compute_u_star(){
	std::vector<double> u_star(_u);
	AmgcMatrixSolver::solve_slae(_mat_u, _rhs_u, u_star);
	return u_star;
}

std::vector<double> Obstacle2DSimpleWorker::compute_v_star(){
	std::vector<double> v_star(_v);
	AmgcMatrixSolver::solve_slae(_mat_v, _rhs_v, v_star);
	return v_star;
}

std::vector<double> Obstacle2DSimpleWorker::compute_p_stroke(
...
std::vector<double> Obstacle2DSimpleWorker::compute_u_stroke_outflow(const std::vector<double>& u_star) const{
	double qin = 0;
	double qout = 0;
	for (size_t j=0; j<_grid.ny(); ++j){
...
std::vector<double> Obstacle2DSimpleWorker::compute_u_stroke(const std::vector<double>& p_stroke, const std::vector<double>& u_stroke_outflow){
...
std::vector<double> Obstacle2DSimpleWorker::compute_v_stroke(const std::vector<double>& p_stroke){
...
double Obstacle2DSimpleWorker::u_i_j(size_t i, size_t j) const{
	size_t ind0 = _grid.yface_grid_index_i_jp(i, j);
	size_t ind1 = _grid.yface_grid_index_i_jp(i, j-1);
	return (_u[ind0] + _u[ind1])/2.0;
}
double Obstacle2DSimpleWorker::u_ip_jp(size_t i, size_t j) const{
	size_t ind0 = _grid.yface_grid_index_i_jp(i, j);
	size_t ind1 = _grid.yface_grid_index_i_jp(i+1, j);
	return (_u[ind0] + _u[ind1])/2.0;
}

double Obstacle2DSimpleWorker::v_i_j(size_t i, size_t j) const{
	size_t ind0 = _grid.xface_grid_index_ip_j(i, j);
	size_t ind1 = _grid.xface_grid_index_ip_j(i-1, j);
	return (_v[ind0] + _v[ind1])/2.0;
}
double Obstacle2DSimpleWorker::v_ip_jp(size_t i, size_t j) const{
	size_t ind0 = _grid.xface_grid_index_ip_j(i, j);
	size_t ind1 = _grid.xface_grid_index_ip_j(i, j+1);
	return (_v[ind0] + _v[ind1])/2.0;
}

double Obstacle2DSimpleWorker::p_ip_jp(size_t i, size_t j) const{
	return _p[_grid.cell_centered_grid_index_ip_jp(i, j)];
}

std::vector<Vector> Obstacle2DSimpleWorker::build_main_grid_velocity() const{
...
Obstacle2DSimpleWorker::Coefficients Obstacle2DSimpleWorker::coefficients() const{
	double sum_cpx = 0;
	double sum_cpy = 0;
	double sum_cfx = 0;
	double sum_cfy = 0;
...
TEST_CASE("Obstacle 2D, SIMPLE algorithm", "[obstacle2-simple]"){
	std::cout << std::endl << "--- cfd24_test [obstacle2-simple] --- " << std::endl;
	...
	Obstacle2DSimpleWorker worker(Re, grid, E);
	worker.initialize_saver(false, "obstacle2");
	...
	Obstacle2DSimpleWorker::Coefficients coefs = worker.coefficients();
	std::cout << "=== Drag" << std::endl;
	std::cout << "Cpx = " << coefs.cpx << std::endl;
	std::cout << "Cfx = " << coefs.cfx << std::endl;
	std::cout << "Cx  = " << coefs.cx  << std::endl;
	std::cout << "=== Lift" << std::endl;
	std::cout << "Cpy = " << coefs.cpy << std::endl;
	std::cout << "Cfy = " << coefs.cfy << std::endl;
	std::cout << "Cy  = " << coefs.cy  << std::endl;

	CHECK(it == 29);
	CHECK(coefs.cx == Approx(4.06).margin(1e-2));
}
